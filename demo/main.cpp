...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        cout << endl << "ODBC data sources:" << endl;
	...
	catch(DbException& ex)
	{
		...
		cout << "Cannot read list of installed drivers:" << endl << ex.what() << endl;
	}
    ...
    {
		b = con.Open(
			//"Driver={SQL Server};Server=localhost;Database=test;Trusted_Connection=Yes;");
			"Driver={SQLite3 ODBC Driver};Database=test.db;");// win32
    ...
    {
		nRetCode = ex.getSqlCode();
		cout << "Cannot drop table:" << endl << statement << endl << ex.what() << endl;
	}
    catch(exception& ex)
    {
        cout << "Cannot drop table:" << endl << statement << endl << ex.what() << endl;
    ...
    {
        cout << "Cannot drop table:" << endl << statement << endl;
    }
	...
	try
	{
		...
		if (sDriverName.find( "psqlodbc.dll") < string::npos)
		{
			...
			::replaceAll( statement, "binary(16)", "bytea");
		}
		...
		nRetCode = query.ExecDirect(
			"insert into person( lfnbr, name, birthdte, balance) values( 1, 'Jack', '1964-03-24', 5432.19)");

		// ... and yet another.
		nRetCode = query.ExecDirect(
			"insert into person( lfnbr, name, birthdte, balance) values( 2, 'Jill', '2004-05-21', 1234.56)");

		// ********************************************************************
		// Now a statements with result set
		// ********************************************************************
		nRetCode = query.ExecDirect("select * from person");
		...
		// ------------------------------------------------
		query.Prepare("insert into person( lfnbr, name, birthdte, balance, pguid) values( ?, ?, ?, ?, ?)");
		...
		nRetCode = query.Prepare(
			"insert into person( lfnbr, name, birthdte, balance, pguid) values( ?, ?, ?, ?, ?)");
		...
		if (colcount > 0)
		{
			...
			for (nRetCode = query.Fetch(); nRetCode != SQL_NO_DATA; nRetCode = query.Fetch())
			{
				...
				string sName;
				query.GetFieldValue("name", sName);	// adressing the column by its column name
				...
				query.GetODBCFieldInfo("balance", fieldinfo);
                ...
                if (varBirthDte.m_pdate)
                    printf("%02d.%02d.%04d\t", varBirthDte.m_pdate->day, varBirthDte.m_pdate->month, varBirthDte.m_pdate->year);
                printf("%8.2f\t", d);
...
*/
		...
		nRetCode = query.Prepare("{CALL testproc(?,?,?,?)}");
		...
		query.SetParamValue( 3, "ABC");
		query.SetParamValue( 4, "DEF", ParamInfo::inputoutput);	// stored procedure changes this parameter
    ...
    {
		nRetCode = ex.getSqlCode();
		cout << "Runtime error:" << endl << ex.what() << endl;
	}
